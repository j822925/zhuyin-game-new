...
async function api(path,data={}){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),8000);try{const r=await fetch(ENDPOINT+'/online/api/'+path,{method:'POST',headers:{'Content-Type':'application/json',...(session?{Authorization:'Bearer '+session.token}:{})},body:JSON.stringify(data),signal:controller.signal});const out=await r.json();if(out.error)throw Error(out.error);return out;}catch(e){if(e.message==='login_required'){session=null;store();disconnect();showLobby();}throw e;}finally{clearTimeout(timeout);}}
function showLobby(){state=null;roomId='';stopAudio();audioKey='';playedKey='';$('recover-room').hidden=true;$('room').hidden=true;$('lobby').hidden=!session;$('login').hidden=!!session;$('logout').hidden=!session;$('identity').textContent=session?session.seat+' 號・'+session.name:'先選自己的座號';if(session)$('my-character').src=avatar(session.avatar);}
...
async function restoreRoom(){const out=await api('me');session={...session,...out.person};showLobby();$('recover-room').hidden=!out.current.room;if(out.current.room){$('recover-room').onclick=()=>enter(out.current.room);message('你還有一間房，可以按「回到剛剛的房間」。');}else $('recover-room').onclick=null;}
...
async function connect(takeover=false){if(!roomId||!session||document.hidden)return;const gen=++socketGeneration;stopped=false;$('connection').textContent='📶 正在連線…';$('reconnect').hidden=true;$('takeover').hidden=true;
 ...
 socket.onmessage=event=>{if(gen!==socketGeneration)return;let d;try{d=JSON.parse(event.data);}catch{return;}
  ...
  if(d.type==='error'){message(messages[d.code]||'畫面正在更新，請稍後再試。');if(d.code==='device_active'){stopped=true;$('takeover').hidden=false;}if(d.code==='login_required'){session=null;store();disconnect();showLobby();}if(state&&['answer_closed','wrong_phase','stale_command'].includes(d.code))render();}
 };
