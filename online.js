...
Object.assign(messages,{cross_class_disabled:'這兩個班級尚未同時開放跨班對戰，請問老師。',common_scope_small:'兩班共同學過的範圍還不夠，請老師確認已教注音。',scope_small:'可出題範圍不足，請老師先設定已教注音。',spelling_unavailable:'班級還沒開放拼音工坊，請問老師。',class_unavailable:'找不到班級，請使用老師提供的網址。'});
...
async function home(){if(roomId&&state&&!['closed','finished'].includes(state.phase)&&!confirm('離開會結束這一場，要回首頁嗎？'))return;try{if(roomId)await api('leave');disconnect();showLobby();message('');location.href=classUrl('./');}catch(e){message(e.message);}}
$('home').onclick=home;$('leave').onclick=home;
$('logout').onclick=async()=>{if(roomId&&!confirm('換人會離開這場對戰，確定嗎？'))return;try{if(roomId)await api('leave');}catch{message('請先確認已離開房間，再換人登入。');return;}session=null;store();disconnect();location.href=classUrl('./');};
...
async function connect(takeover=false){if(!roomId||!session||document.hidden)return;const gen=++socketGeneration;stopped=false;$('connection').textContent='📶 正在連線…';$('reconnect').hidden=true;$('takeover').hidden=true;
 try{const out=await api('ticket',{room:roomId,device});if(gen!==socketGeneration)return;const socket=new WebSocket(classUrl(ENDPOINT.replace(/^http/,'ws')+'/online/socket?room='+encodeURIComponent(roomId)));if(ws){ws.onclose=null;ws.close();}ws=socket;
