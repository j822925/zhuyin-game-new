...
test('1450 processed recordings match release hashes and preserve 89 reused source references',()=>{
 assert.equal(report.sourceCount,1450);assert.equal(report.previousIdentical,164);assert.deepEqual(report.previousDifferent,[]);
 assert.equal(report.files.length,1450);assert.equal(report.files.filter(r=>r.reused).length,89);
...
test('358 exact question replacements preserve tone IDs and 1 legacy fallback',()=>{
...
test('tutor uses all 37 base plus 22 compound recordings',()=>{
 ...
 for(const q of catalog){const clips=tutorClips(q);assert.equal(clips[0],SUPPLIED_TUTOR_AUDIO[q.initial]);assert.equal(clips.at(-1),q.audio);
 const expected=SUPPLIED_TUTOR_AUDIO[q.final]||'audio/compound/c'+String(COMPOUNDS.indexOf(q.final)+1).padStart(2,'0')+'.mp3';
 if(q.final)assert.equal(clips[1],expected);for(const clip of clips)assert.ok(existsSync(new URL('../'+clip,import.meta.url)),clip);
 }
});
...
test('game, exam, review, online and family players all load the new resolver',()=>{
 for(const file of ['app.js','exam.js','exam-review.js','online.js','little-teacher.js','teacher-audio.js'])assert.match(read(file).toString(),/audio-source.js\?v=20260925-originaler1/,file);
