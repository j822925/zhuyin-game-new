...
export function createLittleTeacher({onReturn}){
 ...
 async function playStep(token){
  ...
  parts.forEach((p,i)=>p.classList.toggle('speaking',spokenParts[step]===i&&step<clips.length-1));answer.classList.toggle('speaking',step===clips.length-1);status.textContent=['① 👂','② 👂','③ 👂'][step];
 ...
 return {get open(){return dialog.open;},show(q){stop();clips=tutorClips(q);spokenParts=[q.initial,q.final].map((v,i)=>v?i:-1).filter(i=>i>=0);setVerticalSymbols(parts[0],q.initial);setVerticalSymbols(parts[1],q.final);parts[1].hidden=false;parts[0].setAttribute('aria-label',q.initial||'空白');parts[1].setAttribute('aria-label',q.final||'空白');showSpellingTone(answer,q);dialog.showModal();replay();},close:()=>close(false)};
}
