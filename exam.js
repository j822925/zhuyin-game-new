...
import {createExamReview} from './exam-review.js?v=20260925-originaler1';
...
const key=()=>classStorageKey(`zhuyin.exam.pending.${examId}.${seat}`);
...
async function request(d){const out=await auth.request({...d,seat,examId,catalogVersion:3});if(out.error)throw Error(out.error);return out;}
...
function enable(){const enabled=heard&&!busy&&!readPending()&&state&&!state.finished&&!state.review&&Date.now()+offset<state.exam.ends;document.querySelectorAll('[data-choice]').forEach(b=>b.disabled=!enabled);$('submit').disabled=!enabled||!(state?.exam.mode==='single'?selection.answer:selection.initial!==undefined&&selection.final!==undefined);}
