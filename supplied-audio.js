...
function render(){
 ...
 for(const section of ['units','syllables']){
  ...
  for(const r of rows[section]){
   ...
   const values=section==='units'?[r.label,r.available?'已替換':'缺少，沿用原音']:[r.id,r.label+(r.word?' · '+r.word:''),(r.available?'已替換':'缺少，沿用原音')+(r.enabled?'':'（暫不出題）')];
